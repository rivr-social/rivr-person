...
export async function updateProfileAction(
  ...
  // Resolve the active actor: controller by default, or a persona row when
  // the active-persona cookie is set and the persona is owned by the
  // controller. Writes target the persona's `agents` row so bio, name,
  // socials, etc. live with the persona itself; email stays bound to the
  // controller because it is the auth identity.
  ...
  // Settings writes are rate-limited per controller to reduce abuse — sharing
  // the bucket across personas prevents one account from stacking N personas
  // worth of rate budget.
  const check = await rateLimit(`settings:${controllerId}`, RATE_LIMITS.SETTINGS.limit, RATE_LIMITS.SETTINGS.windowMs);
  if (!check.success) {
  ...
  // Email is the controller's auth identity. Only validate the field when the
  // controller is the active actor; for persona writes the email column on
  // the persona row is left untouched (personas share the controller's
  // session and cannot independently change auth email).
  if (!isPersona) {
    if (!email) {
  ...
  try {
    ...
    // Email changes only apply when the controller is the active actor.
    // Persona writes never change the persona row's email column.
    const emailChanged = !isPersona && previousEmail !== email;
    ...
    // For personas, the email column is left at its current row value so we
    // never accidentally write controller-owned auth identity onto a persona.
    if (isPersona) {
    ...
    scheduleEmbedding(() => embedAgent(actorId, name, bio || undefined));

    // Sync display name to Matrix profile (fire-and-forget)
    if (name !== current.name) {
      void syncProfileToMatrix(actorId, { displayName: name });
    }

    if (input.murmurationsPublishing === true) {
      void syncMurmurationsProfilesForActor(actorId).catch((err) => {
...
export async function updateProfileImageAction(
  ...
  // Persona-aware: image writes target the active actor's row so personas
  // get their own avatar/cover without affecting the controller's images.
  const activeActor = await resolveActiveActorAgentId();
  if (!activeActor) {
    return { success: false, error: "You must be logged in." };
  }

  const { actorId, controllerId } = activeActor;
  ...
  try {
    ...
    if (field === "avatar") {
      await db.execute(sql`
        ...
        WHERE id = ${actorId}
      `);
      void syncProfileToMatrix(actorId, { avatarUrl: normalizedUrl });
    } else {
      const [current] = await db
